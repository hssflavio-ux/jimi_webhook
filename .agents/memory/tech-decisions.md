...
- **Paginação**: usar `report_pagination($page,$totalPages,$totalRows,$unit)` — NUNCA o laço `for ($i=1; $i<=min($totalPages,10); $i++)` que estava copiado em 7 telas (a partir da página 11 não mostrava a página atual). O helper faz janela deslizante, tira `export=` dos links e preserva os filtros da URL.
- **Faixa horária opcional**: usar `report_time_window($col,$df,$dt,$tf,$tt,$mode)`. Duas semânticas, escolhidas pelo usuário na tela (`time_mode`): **'continua'** = uma janela só, `date_from time_from` → `date_to time_to` (`brt_datetime_range_to_utc()`); **'diaria'** = dias inteiros no BETWEEN + `TIME(CONVERT_TZ(col,'+00:00','-03:00'))` filtrando dentro de cada dia do período. No modo diário, `time_from > time_to` = janela que cruza a meia-noite (turno da noite, ex.: 22:00–06:00) → `OR`, nunca `BETWEEN` (senão o resultado volta vazio). Implementado em `rel_posicoes`; `rel_deslocamento` ainda só tem o modo contínuo.
