...
- **Faixa horária opcional** (posições e deslocamento): `time_from`/`time_to` + `brt_datetime_range_to_utc()`. Semântica é **janela contínua** (`date_from time_from` → `date_to time_to`), NÃO "essa faixa em cada dia do período" — se um dia pedirem por-dia, é outra query (precisa de `TIME(CONVERT_TZ(...))` no WHERE).
