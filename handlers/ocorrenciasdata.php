...
if ($search) {
    $where .= ' AND (o.imei LIKE :q OR d.name LIKE :q2)';
    $params[':q'] = "%$search%";
    $params[':q2'] = "%$search%";
}
...
$devStmt = $db->prepare("
    ...
    FROM devices d2
    WHERE 1=1 " . ($customerId ? 'AND d2.customer_id = :cid2' : '')
);
...
$countStmt = $db->prepare("
    SELECT COUNT(*) FROM occurrences o $where $whereDates
");
...
foreach ($rows as $r) {
    $data[] = [
        ...
        'first_alarm_at'  => $r['first_alarm_at'],
...
echo json_encode([
    ...
    'data' => [
        'kpis' => [
            'total'       => (int)($kpis['total'] ?? 0),
            'aguardando'  => (int)($kpis['aguardando'] ?? 0),
            'em_tratativa'=> (int)($kpis['em_tratativa'] ?? 0),
            'resolvida'   => (int)($kpis['resolvida'] ?? 0),
