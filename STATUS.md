...
> **Última atualização**: 23/07/2026 — **(a) Deploy "não acessa o banco com as credenciais do .env": diagnóstico falso — as migrations conectam normalmente; os 2 avisos eram misdiagnósticos (FASE 1 testava sem credenciais; FASE 2 abortava o `mysqldump` por 2 VIEWs órfãs quebradas — `vw_alarm_types_*_codes` → tabela inexistente `alarm_types_reference`). VIEWs dropadas no homolog (dump completa) + `deploy.sh` corrigido (§12.18). (b) Fix da segmentação de viagens: `trip_builder.php` encerrava o deslocamento só em `acc=desligado`, então FJR7B59 (`869058070151343`) colapsava a jornada num deslocamento único de ~23h; agora segmenta também por parada sustentada e buraco de dados → 11 → 39 viagens no homolog. Commitado/deployado (`acf09f3`, servidor já em `HEAD=acf09f3`) (§12.17).** ⚠️ **Só o fix do `deploy.sh` de (a) segue NÃO commitado/deployado** (exige as 2 passadas, §12.16); as VIEWs órfãs já foram dropadas no homolog. Anterior: Deslocamento em 2 modalidades + mapa de rota + teto de 31 dias + índice composto `trips` em `4.3.0` (§12.16).
...
### 12.18 Deploy "não acessa o banco com as credenciais do .env" — diagnóstico falso (23/07/2026)
...
- **PENDÊNCIAS**: (1) o fix do `deploy.sh` ainda **não foi commitado/deployado** (junto com o do `trip_builder`, §12.17) — como altera o próprio `deploy.sh`, exige as **2 passadas** (§12.16). (2) **Produção**, se existir e for anterior a 06/07/2026, provavelmente tem as mesmas 2 VIEWs órfãs → rodar o mesmo `DROP VIEW IF EXISTS` lá.
