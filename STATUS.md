...
>
> ### Deploy v4.4.1 no homolog — 28/07/2026, `2d32164`
> **Publicado e verificado.** Código em `main`/`2d32164`, working tree limpo, schema em **4.4.1** (`notification_rules`, `notifications`, `smtp_settings` criadas; `jobs.type` com `notification`; `jobs.attempts` presente; 6 regras globais de seed), `.env` com `SYSTEM_VERSION=4.4.1`, `NOTIFY_ENABLED=1` e `APP_KEY` de 64 hex gerada no servidor. Lint 0 erros; `/config-smtp` e `/config-notificacoes` respondem 302 (existem, exigem login), `/notificacoesdata` responde 200; Apache/MySQL/PHP-FPM ativos e os 4 workers rodando.
> **Higiene aplicada**: removidos `includes/config.php` (resquício com `DB_PASS` em texto puro apontando para um banco inexistente `jimi_webhook`; confirmado que nada no código o carregava) e `.env.bak-20260708_215709`. O `handlers/pushterminalrealtimestatus.php` deixou de ser arquivo-fantasma: foi documentado e versionado (`f55a561`).
> ⚠️ **Armadilha encontrada e corrigida no caminho** (`2d32164`): o `deploy.sh` **se auto-substituiu durante a própria execução**. O bash lê o script do disco conforme executa; o `git pull` da FASE 3 trocou o arquivo e o interpretador seguiu a partir do offset antigo dentro do arquivo novo — **as migrações v4.4.0 e v4.4.1 nunca rodaram e o deploy imprimiu "CONCLUÍDO" com exit 0**, deixando código novo sobre banco 4.3.0. O script agora compara o próprio sha256 antes/depois do pull e se re-executa uma vez (guarda `DEPLOY_REEXEC` contra laço). Ao publicar algo que altere o `deploy.sh`, o padrão seguro é dar o `git pull` **separado** e só então chamar o script.
> 📌 **Latente, não corrigido**: o gate de versão das migrações usa `!= "X"` por bloco, então toda publicação reaplica a cadeia inteira a partir da v4.2.1 (`4.4.1 → aplica v4.2.1 → 4.2.1 → aplica v4.3.0 → …`). É inofensivo porque as migrações são idempotentes, mas as mensagens "versão atual" ficam enganosas e o custo cresce a cada release.
> 📌 **Cosmético**: `/ping` ainda responde `"version":"2.0.0"` — string fixa no `handlers/ping.php`, não lê `SYSTEM_VERSION`.
>
