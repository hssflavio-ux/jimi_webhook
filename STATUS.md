...
| 15 | `/comandos` retorna HTTP 500 após correção #14 | Uso de `mb_substr()` (extensão `mbstring` não disponível no servidor) na nova coluna Resposta da tabela de histórico | Substituído `mb_substr()` por `substr()`. Cadeia `??` trocada por `isset()` para consistência com o padrão do projeto | `comandos.php` |
