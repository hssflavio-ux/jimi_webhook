...
> **Última atualização**: 22/07/2026 — **Relatório de Deslocamento em 2 modalidades (por deslocamento | fechamento diário) + mapa de rota por deslocamento (balões partida/chegada, pontos de comunicação, ocorrências destacadas) + teto global de 31 dias em todos os relatórios + índice composto `trips(customer_id, started_at)`. Commit `5f6b8ed`, DEPLOYADO e VERIFICADO no homolog: banco em `4.3.0`, índice criado / redundante removido, rotas 302 (§12.16).** Anterior: fix do `trip_builder`/`worker`/`metrics_rollup` (liam `gps_data.ignition` inexistente) + anti-fragmentação + filtro de qualidade — câmera 182 (FJR7B59) com 10 viagens reais (§12.15); Wiki reescrita para o usuário final (§12.14).
