...
> 4. **`APP_KEY` e SMTP**: o envio agendado depende de credenciais cadastradas em `/config-smtp` (ou no `.env` como fallback) e de **`APP_URL` correta** — é a base do link quando o anexo passa de `MAIL_MAX_ATTACH_MB`. O envio real **nunca foi validado contra um provedor externo**; localmente foi validado contra um SMTP de captura.
...
>
> **Próximo passo do roadmap**: **Fase 5 — atualizar a Central de Ajuda (`/wiki`)**, que está congelada na v4.3.0 e precisa cobrir as quatro fases. É o último passo da iniciativa, por decisão registrada (fazer de uma vez evita reescrever as mesmas seções quatro vezes). Checklist detalhado em **§12.6**.
>
