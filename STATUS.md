...
> **Última atualização**: 22/07/2026 — **Fix do Relatório de Deslocamento: `trip_builder`/`worker`/`metrics_rollup` liam `gps_data.ignition` (inexistente) em vez de `acc` → tabela `trips` nunca populava; + guard anti-fragmentação + filtro de qualidade. Deployado no homolog (`63b686c`) e verificado: câmera 182 (FJR7B59) com 10 viagens reais (§12.15).** Anterior: Wiki/Central de Ajuda reescrita para o usuário final (§12.14); fix do vídeo automático dos alarmes (34818→37384) + exibição na ocorrência (§12.13, aguardando deploy).
...
- **Pendências**: exibir aviso de clamp também no `/exportar` (hoje só encurta silenciosamente o job); considerar rollup `trip_days` se um dia o fechamento diário precisar de janelas > 31 dias.
