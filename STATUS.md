...
- **Faixa horária no `/relatorios/posicoes`** (pedido do usuário: "escolher a data e a hora fica opcional"): campos `time_from`/`time_to`, vazios = `00:00`/`23:59`. Usa `brt_datetime_range_to_utc()` — a **mesma semântica de janela contínua** já adotada no Relatório de Deslocamento (de `date_from time_from` até `date_to time_to`, e **não** "essa faixa em cada dia"). Vale também para o export, cujo subtítulo cita a faixa.
...
- **Pendente**: commit/push e deploy no homolog.
