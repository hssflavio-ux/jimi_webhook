...
> **Última atualização**: 23/07/2026 — **UX dos relatórios: botão `← Voltar` + ordenação crescente por data/hora com seta clicável em todos os relatórios** (helpers `report_sort_params()`/`report_sort_link()`/`report_back_button()` em `includes/functions.php`; 6 telas verificadas localmente, lint OK). **Ainda não commitado/deployado** (§12.19).
