...
### Added
- **Faixa horária opcional no Relatório de Posições** (`/relatorios/posicoes`): novos campos `time_from`/`time_to` ao lado do período — o usuário escolhe as datas e, opcionalmente, a hora inicial/final (vazio = 00:00 / 23:59). A janela é contínua (`date_from time_from` → `date_to time_to`, via `brt_datetime_range_to_utc()`, mesma semântica já usada no Relatório de Deslocamento) e vale também para o export, cujo subtítulo passa a citar a faixa.
