...
- **Adicionado** seis seções à wiki, escritas a partir dos handlers: **Manutenção** (limiares 200 km / 10 h / 7 dias, aba Documentos com CNH e toxicológico, aviso diário pelo sino e por e-mail), **Painel** (widgets, layout por usuário, período Hoje/7 dias/Mês), **Auditoria** (as quatro abas, filtros, teto de 31 dias, exportação), **Comandos por SMS** (1 crédito por equipamento, limite de 160 caracteres, permissão própria, colunas Envio/Entrega/Resposta), **SMS** (configuração da conta da plataforma, método de resposta, teste de credencial) e **Configurações IA** (leitura × aplicação, perfil de leitura completa, confirmação que sobrescreve). O registro passa de 42 para 48 seções e a última exceção `PENDENTE etapa 2` de `tests/helpers/wiki_registry.test.php` foi removida.
- **Adicionado** à seção Primeiros Passos: **Esqueci minha senha**, a tela **Defina sua senha** (senha temporária) e a faixa de operação como cliente (visão do revendedor); em Usuários, senha em branco, **Reenviar senha** e os selos; em Clientes, o botão **Entrar como** e a faixa de impersonação.
- **Adicionado** às seções Ativos, Chips SIM e Equipamentos o fluxo real **chip → câmera → veículo** (vigente desde a v4.11.0: instalação exige chip, veículo existe sem câmera, histórico de instalações preservado, dono da câmera derivado do veículo) e os rastreadores **JM-VL01/JM-VL02** (sem câmera, sem vídeo, sem alertas de IA).
- **Adicionado** em `tests/helpers/wiki_registry.test.php` a verificação de que os **três relatórios-irmãos da Auditoria** (`auditoria_negados`, `auditoria_cadastro`, `auditoria_login`) também têm `require_admin();` — a tela-mãe já era conferida pelo registro, mas os irmãos abrem por URL direta e não têm seção própria.
- **Alterado** 🔴 **MUDANÇA DE ACESSO — a Auditoria passa a ser exclusiva do administrador do sistema** (decisão do dono do produto, 20/09/2026). `require_admin()` nos 4 handlers (`auditoria`, `auditoria_negados`, `auditoria_cadastro`, `auditoria_login`); o item sai do grupo Cadastros e vai para o menu inferior com `admin_only` (item de grupo não respeita `admin_only`); a matriz de permissões mostra o rótulo "Auditoria (só admin)". **Quem não é administrador — inclusive o revendedor e o usuário sem grupo de permissão — deixa de ver e de abrir a Auditoria**; marcar a tela num grupo não a libera mais. A wiki acompanha: badge "admin", e o filtro Cliente e a visão de todos os clientes valem só para o administrador.
- **Alterado** **Manutenção** passa a mostrar a **placa do veículo onde a câmera está instalada agora** (`vehicles.plate` da instalação aberta), na lista, no seletor e na pesquisa. Antes lia `devices.device_name` (legado congelado desde a v4.11.0) e a câmera cadastrada depois disso aparecia como "(sem placa) IMEI". A placa só vale se o veículo for do **mesmo cliente do lembrete**; senão cai no nome legado / "(sem placa)".
- **Alterado** **Equipamentos, importação de câmeras em lote**: a janela mostra o resultado real ("N importado(s), M ignorado(s)" e os avisos por linha) em vez de sempre dizer "Importação concluída", e a lista só recarrega ao fechar a janela, para o resultado não sumir.
- **Alterado** **Chips**: "Remover" recusa chip vinculado a uma câmera, com a mesma regra e mensagem de "Desativar" (cita o IMEI e aponta para `/equipamentos`); a mensagem de "Selecione o cliente" passa a apontar o seletor do topo do menu lateral, porque o formulário não tem campo de cliente. O exemplo de filtro de ação da Auditoria passa a ser uma ação real (`sim_card.delete`, antes `chip.delete`, que não existe).
- **Corrigido** (wiki) seções que descreviam comportamento antigo: **Desatualizados** (dois grupos; último sinal de comunicação, tolerância 5 min ligada / 30 min desligada), **Deslocamento** (Distância, Hodômetro e Horímetro, sem estimativa por GPS) e **Posições** (sem o mapa do período; com Hodômetro e Km rodado), **Dashboard de Ocorrências** / tratativa (indicadores, filtros e colunas reais, número e velocidade de cada alarme, ocorrências sem vídeo, falso positivo), **Alertas Videomonitoramento × Dirigibilidade** (condução, câmera e rastreador), **Comandos** (a tela só avisa sobre o modelo, não trava — assim é desde a v4.17.28), **Grupos de Permissão** (sem número fixo de telas; lista das telas só do administrador), **Resumo** e **Rastreamento** (refresh de 30 s, estados, visão por clientes do revendedor), **Motoristas** (motorista corrente), **Vídeo ao vivo**, **Playback**, **Downloads**, **Agendamentos** e **Exportar**.
- **Segurança** Manutenção: a placa lida do veículo que detém a câmera **agora** vazaria a de um veículo de outro cliente (câmera reatribuída, ou IMEI forjado no salvar) — o `LEFT JOIN vehicles` ganhou `AND v.customer_id = r.customer_id` nas duas consultas. Chips: o `SELECT` do "antes" de "Remover" passou a respeitar o escopo do cliente, senão a mensagem da nova guarda mostraria o IMEI vinculado ao chip de **outro** cliente.
- **Nota** achados desta rodada que **não foram corrigidos** (decisão do dono do produto) estão listados em `STATUS.md`: escopo de tenant em Manutenção, Chips, Resumo, tratativa de ocorrência e "Aplicar em outras câmeras"; "Placa" das telas de operação lendo o nome legado; "Online" com 10 min em Ativos e 5 min em Equipamentos; arestas da importação de câmeras.
- **Verificação**: `php tests/helpers/wiki_access.test.php` (122 verificações) e `php tests/helpers/wiki_registry.test.php` (11 verificações), 0 falhas; `php -l` limpo no projeto. **Sem conferência no navegador e sem execução das telas alteradas** (Manutenção, importação de câmeras, Chips, menu, `/wiki` nos três perfis) nem do Playwright — não há MySQL/`.env` local; confirmar em homolog após o deploy.
...
Em `/relatorios/deslocamento` a coluna **Distância** passou a mostrar o hodômetro final menos o inicial do trecho (o que a coluna "Hodômetro" já calculava); a coluna **Hodômetro** passou a mostrar a leitura ABSOLUTA do contador no fim do trecho (nova). As duas são recalculadas AO VIVO a partir de `gps_data.mileage`, nunca lidas de `trips.distance_km` — por isso valem também para viagens antigas, sem precisar de backfill.

- **Corrigido** `scripts/trip_builder.php`: `trips.distance_km` (persistido daqui pra frente) passou a ser o delta de hodômetro do trecho (`odometer_delta_from_points()`), NULL quando o equipamento não reporta leitura válida — nunca GPS. O filtro de qualidade `isRealTrip()` continua em GPS/velocidade de propósito (variável interna `gps_distance_km`, não persistida): equipamento com hodômetro temporariamente inválido não pode deixar de REGISTRAR uma viagem real.
- **Corrigido** `scripts/state_builder.php`: `device_state_segments.distance_km` passou a ser o delta de hodômetro do segmento, mesma regra. Sem efeito visível hoje — as duas telas que leem essa tabela (`/relatorios/paradas`, `/relatorios/ociosidade`) têm a coluna de distância desligada (`showDist=false`) e `rel_ignicao.php` seleciona a coluna sem exibi-la.
- **Corrigido** `handlers/pushgps.php` (`calculateDistance()`): alimenta `gps_data.distance_from_previous` e o acumulador `device_statistics.total_distance_km` (stored procedure inalterada, só a fonte do valor mudou) com o delta de hodômetro desde a última leitura válida (por `gps_time`, não por ordem de chegada — pontos atrasados existem). Sem hodômetro válido, sem base de comparação ou delta negativo (reset do sensor): 0, sem fallback pra GPS.
- **Corrigido** `handlers/rel_deslocamento_replay.php`: o KPI "Distância" do resumo lia `trips.distance_km` (GPS); passou a recalcular do hodômetro sobre os mesmos pontos já carregados para o mapa — mesma fonte que o contador "Percorrido" ao vivo já usava, eliminando uma divergência entre os dois na MESMA tela. **Achado no caminho**: esse contador ao vivo tratava `mileage=0` como leitura válida (`odometer_km(0) = 0.0`, não null) — equipamento sem hodômetro real mostrava "0.0 km" fixo em vez de "—" a viagem inteira; corrigido descartando `mileage <= 0` antes de converter, e o baseline (`mileage0`) agora busca a primeira leitura VÁLIDA entre os pontos amostrados, não cegamente o primeiro ponto.
- **Corrigido** `handlers/rel_deslocamento_rota.php`: mesmo recálculo pelo hodômetro sobre os pontos já carregados para o mapa (viagem única ou fechamento diário), no lugar de `trips.distance_km`/`SUM(trips.distance_km)`.
- **Corrigido** `scripts/worker.php` (relatório agendado "trips" — export por e-mail, `case 'trips'`): mesma subquery ao vivo de `rel_deslocamento.php` no lugar de `t.distance_km` — sem isso, essa tela era a única a continuar mostrando GPS mesmo depois da correção acima.
- **Removido** `calculate_distance()` (`includes/functions.php`) — lei dos cossenos esférica, única chamadora era `pushgps.php`, que passou a usar hodômetro. Função morta desde este commit; o comentário de `includes/geofence.php` que alertava contra usá-la foi atualizado (a função não existe mais para alertar contra).
- **Adicionado** `odometer_delta_from_points()` (`includes/functions.php`): varredura compartilhada por `trip_builder.php`/`rel_deslocamento_rota.php`/`rel_deslocamento_replay.php` — primeira e última leitura de `mileage` válida (>0) num array de pontos já carregado, delegando a `odometer_delta_km()` existente.
- **Decisões do dono do produto** (não implementadas por escolha, registradas para não serem "descobertas" de novo): sem fallback para GPS quando o hodômetro está inválido (mostra "—"); sem backfill de `trips`/`device_state_segments` já gravados (só viagens/segmentos novos usam hodômetro — os relatórios que recalculam ao vivo não são afetados por essa decisão, só os que leem a coluna persistida direto, como o "trips" agendado do worker); filtro de detecção de viagem (`isRealTrip()`) continua em GPS/velocidade.
- **Verificação**: `php -l` limpo no projeto inteiro; `tests/helpers/odometro_horimetro.test.php` estendido com 6 casos novos para `odometer_delta_from_points()` (27/27 passando); suíte de helpers sem banco rodada por completo, nada quebrou. **Sem MySQL nesta sessão** — nenhuma linha nova de `trips`/`device_state_segments`/`gps_data` foi gerada e conferida contra webhook real; `scripts/trip_builder.php`, `scripts/state_builder.php` e `handlers/pushgps.php` não foram exercitados contra banco. Precisa de conferência em homolog/produção antes do próximo deploy.
...
- **Adicionado** tela **Alertas Dirigibilidade** (`/relatorios/dirigibilidade`, permissão `relatorios`): a mesma grade de `rel_alarmes.php` num modo (`handlers/rel_dirigibilidade.php`), sem coluna Vídeo, sem "Pedir vídeo", sem modo diagnóstico; modelos salvos com chave própria.
...
- **Adicionado** tipo `driving_alarms` ("Alertas Dirigibilidade") em Agendamentos e Exportar.
...
- 🔴 **Corrigido, mesmo dia** — `scripts/backfill_driver_sessions.php` estourava a FK `fk_ds_driver` em produção: assumia que `alarms.driver_id` já era um `drivers.id` válido, mas isso só vale para alarmes gravados DEPOIS do fix em `pushalarm.php` — o identifier bruto de alarmes mais antigos nunca tinha sido resolvido contra `drivers.identifier`. Corrigido chamando `resolve_driver_by_identifier()` (a mesma função do caminho em tempo real) antes de cada inserção; identifier sem motorista cadastrado agora é contado e pulado, nunca crasha. **Achado no caminho, raiz do problema**: `drivers` está com zero linhas em produção — o mecanismo (tempo real e backfill) está correto, mas mudo até alguém cadastrar os motoristas em `/motoristas` com `identifier` batendo com o que a câmera manda.
...
Pedido do dono do produto, retomando os 4 itens que a varredura de fidelidade ao design system (mesma data) tinha deixado como "decisão de produto, não implementada".

- 🔴 **Removido** — as abas **Comandos** e **Configurações** de `/ativos/{id}` (`handlers/ativo_detalhe.php`), a UI antiga com `proNo`/`serverFlagId`/JSON cru, redundante com `/comandos` e `/parametros`, que já modernizaram a mesma função. `?tab=comandos`/`?tab=configuracoes` agora **redirecionam** (`header('Location:')`, antes de qualquer HTML) para `/comandos?imei=` e `/parametros` respectivamente — a sidebar (`web/layout_ativo_sidebar.php`) troca as duas abas por links diretos para lá. **Correção em relação ao que a varredura tinha registrado**: o destino de "Configurações" NÃO é `/configuracoes-ia` — essa tela é proNo 128, só JIMI (ADAS/DMS/velocidade), protocolo diferente do `33027`-`33031` (JT/T) que a aba antiga mandava; o destino certo é `/parametros`, a área dedicada de parametrização JT/T (que inclusive avisa que 33027-33030 estão pausados no firmware atual do fabricante — motivo a mais para não duplicar a funcionalidade numa segunda tela).
- **Adicionado** busca + paginação em `/manutencoes` (aba Manutenção), no mesmo padrão de `motoristas.php`/`chips.php`/`ativos.php` — única tela do grupo Cadastros com lista+painel que ainda não tinha.
- **Aplicado** `.filtro-campo`/`.filtro-rotulo` (criadas na v4.9.38, ignoradas desde então) nos campos de filtro/busca reais de `equipamentos.php`, `rastreamento.php`, `motoristas.php`, `clientes.php`, `chips.php`, `ativos.php`, `usuarios.php`, `manutencoes.php`, nos 9 `rel_*.php` com filtro de formulário (`rel_posicoes`, `rel_deslocamento`, `rel_desatualizados`, `rel_alarmes`, `rel_ocorrencias`, `rel_geocercas`, `rel_ignicao`, `rel_velocidade`, `rel_status_frota`), no helper compartilhado `report_device_select()` (`includes/functions.php`) e em `includes/report_segments.php` (cobre `rel_paradas`/`rel_ociosidade`). **Dois falsos-positivos da varredura descartados** após conferência: `geocercas.php` e `config_ocorrencias.php`/`config_notificacoes.php` não têm barra de filtro — os `<select>` que apareceram na varredura são campos de formulário de cadastro (`.form-group`), não filtro de listagem, e `.filtro-campo` não se aplica a eles.
- **Alterado** feedback assíncrono de `painel.php` (erro ao salvar layout de widgets) e `config_dispositivos.php` (validação de ID de parâmetro, resultado do reinício de terminal) — `alert()` nativo trocado por mensagem inline junto da ação, no mesmo padrão que essas telas já usam para outros resultados (`cfg-query-result`/`cfg-set-result`). De quebra, `resumo.php` e `rastreamento.php` ganharam o indicador de "atualizado HH:MM:SS" que `ocorrencias_dashboard.php` já tinha no polling de 30s — as duas únicas telas do mesmo padrão que ficavam sem indicação de quando os dados na tela foram atualizados pela última vez.

## [Unreleased] — 4.18.2

**`/configuracoes-ia`: textos revisados, quadros alinhados ao design system, grade sem buracos, e perfil de leitura completa (export entre câmeras do mesmo modelo + `writeconfig.txt`).**

- **Alterado** os dois textos de estado vazio da tela ("Selecione o equipamento para verificar e configurar sua IA." / "O modelo não tem configuração disponível.") — pedido do dono do produto.
- 🔴 **Corrigido** `.ia-cell`/`.ia-param input` usavam raio, sombra de hover e cores fora dos tokens do design system (`border-radius:10px` em vez de `var(--radius-lg)`, sombra de hover custom em vez de `var(--shadow-soft)`, input sem foco azul). **Mesma divergência corrigida também em `.param-cell`/`.param-edit input` (`handlers/ativo_detalhe.php`, aba Parâmetros de `/ativos/{imei}`)**, de onde `/configuracoes-ia` tinha herdado o padrão original — incluindo `var(--line,#e5e7eb)`, variável que não existe em `:root` (sempre caía no fallback hardcoded). Ambas alinhadas aos tokens já usados por `.card`/`.kpi-item`/`.form-group input:focus` em `web/layout_base.php`.
- **Corrigido** `.ia-grid` ganhou `grid-auto-flow: dense` — os quadros combinados EVENTSET+EVENTALERT (`grid-column: span 2`) deixavam buraco na linha quando não cabiam por inteiro (auto-placement sem `dense` não preenche células vazias com itens menores que vêm depois); agora só a última linha pode ficar incompleta.
- **Adicionado** perfil de leitura completa (`device_ia_config_snapshots`, migração `v4.18.2`): ao final de "Ler tudo agora", os comandos são reconstruídos a partir da resposta de cada consulta (`EVENTSET,ALDW#` → `EVENTSET,ALDW#,60` → `EVENTSET,ALDW,60#` — forma medida em `includes/ia_config_catalog.php`) e salvos como o perfil do equipamento. A tela passa a mostrar "Última leitura completa em DD/MM/AAAA HH:MM — N de M comando(s)" quando o equipamento é selecionado, com dois botões: **baixar `writeconfig.txt`** (um comando por linha, sem comentário — recurso real da câmera, confirmado pelo dono do produto: ela lê o cartão micro SD e aplica a configuração sozinha) e **aplicar em outras câmeras do mesmo modelo** (envio ao vivo, comando a comando, com prévia da lista e confirmação antes de disparar).
- 🔴 **Corrigido, varredura completa do app** — 7 variáveis CSS "fantasma" (`var(--brand)`, `var(--danger)`, `var(--accent)`, `var(--radius)`, `var(--sidebar-bg)`, `var(--surface-2)`, `var(--font-mono, monospace)`), sobras da paleta anterior ao rename para o design system atual, sem definição em `:root` — a propriedade inteira era ignorada pelo navegador (raio/cor/sombra simplesmente ausentes, não errados), então nunca apareciam como "cor errada" numa checagem visual rápida. Afetava **11 arquivos**: `/auditoria`, `/auditoria/negados`, `/auditoria/cadastro`, `/auditoria/login` (aba ativa sem destaque), `/comandos` (seleção, chip de exemplo, foco por teclado invisíveis), `/relatorios/status-frota` (anel do filtro ativo), `/ativos/{imei}`, `/parametros`, `/configuracoes/parametros`, `/relatorios/parametros` (mensagens de erro sem vermelho), `/geocercas` (link sem azul), `/wiki` (13 caixas de mockup com cantos quadrados + sidebar de exemplo sem fundo escuro), `/comandos-sms` (linha bloqueada sem destaque), e o **painel de notificações global** (cantos quadrados + tipografia mono genérica em vez de JetBrains Mono). Resolvidas para os tokens reais; `.cmd-item.sel` em `/comandos` também trocou um `#e8f0ff` hardcoded por `var(--primary-soft)` (o token já existente, quase idêntico). Método: todo `var(--...)` usado no app comparado contra as variáveis realmente definidas em `web/layout_base.php`.
...
- 🔴 **Correção do dono do produto (09/09/2026): com a parametrização livre, aridade não trava mais nada — bastava incluir o modelo em `modelos`.** Reexaminados os 6 "gaps" listados acima: `FATIGUE`, `SENALM` e `SERVER` **já tinham** JC181 numa entrada existente (a auditoria tinha comparado com a variante errada); `POWERALM` e `EXBATALM` **de fato** faltavam JC181 — corrigido; `SOSALM` tinha JC181/JC182 em `consulta_modelos` mas não em `modelos` — inconsistência corrigida.
- 🔴 **`REBOOT#`/`RESET#`/`RESTART#` consolidados numa entrada só** (decisão do dono do produto): eram três comandos para a mesma ação de reiniciar, cobrindo subconjuntos de modelos — com a lista livre, três itens fazendo a mesma coisa é só ruído. `REBOOT#` vira a entrada canônica, com `modelos` = união das três (JC371/JC450/JC182/JC400D/JC400AD + JM-VL01/JM-VL02 do RESET# + JC181, cujo doc só documenta `RESET`). Total do catálogo: 237→235 entradas, 168→166 comandos distintos (cabeçalho do arquivo e `tests/helpers/command_response.test.php` atualizados).
